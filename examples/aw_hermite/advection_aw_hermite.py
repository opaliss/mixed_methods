"""Module to run Hermite linear advection testcase

Author: Opal Issan
Date: Nov 17th, 2025
"""
import sys, os

sys.path.append(os.path.abspath(os.path.join('..')))

from operators.aw_hermite.setup_aw_hermite import SimulationSetupHermite
from operators.implicit_midpoint_adaptive_single_stream import implicit_midpoint_solver_adaptive_single_stream
import time
import numpy as np


def rhs(y):
    A_e = setup.alpha_e[-1] * setup.A_off + setup.u_e[-1] * setup.A_diag + setup.nu * setup.A_col
    # evolving only electrons
    return A_e @ y


if __name__ == "__main__":
    for Nv in [20, 50, 100, 200, 400]:
        setup = SimulationSetupHermite(Nx=101,
                                       Nv=Nv,
                                       epsilon=1,
                                       alpha_e=np.sqrt(2),
                                       alpha_i=np.sqrt(2 / 1836),
                                       u_e=0,
                                       u_i=0,
                                       L=2 * np.pi,
                                       dt=1e-2,
                                       T0=0,
                                       T=20,
                                       nu=0,
                                       u_tol=np.inf,
                                       alpha_tol=np.inf)

        # initial condition: read in result from previous simulation
        y0 = np.zeros(setup.Nv * setup.Nx)
        # first electron 1 species (perturbed)
        x_ = np.linspace(0, setup.L, setup.Nx, endpoint=False)
        y0[:setup.Nx] = (1 + setup.epsilon * np.cos(x_)) / setup.alpha_e

        # start timer
        start_time_cpu = time.process_time()
        start_time_wall = time.time()

        # integrate (implicit midpoint)
        sol_midpoint_u, setup = implicit_midpoint_solver_adaptive_single_stream(y_0=y0,
                                                                                right_hand_side=rhs,
                                                                                a_tol=1e-10,
                                                                                r_tol=1e-10,
                                                                                max_iter=100,
                                                                                param=setup)

        end_time_cpu = time.process_time() - start_time_cpu
        end_time_wall = time.time() - start_time_wall

        print("runtime cpu = ", end_time_cpu)
        print("runtime wall = ", end_time_wall)

        # save the runtime
        np.save("../../data/aw_hermite/advection/sol_runtime_Nv_" + str(setup.Nv) + "_Nx_" + str(setup.Nx)
                + "_" + str(setup.T0) + "_" + str(setup.T), np.array([end_time_cpu, end_time_wall]))

        # save results
        np.save("../../data/aw_hermite/advection/sol_u_Nv_" + str(setup.Nv) + "_Nx_" + str(setup.Nx) + "_"
                + str(setup.T0) + "_" + str(setup.T), sol_midpoint_u)

        np.save("../../data/aw_hermite/advection/sol_t_Nv_" + str(setup.Nv) + "_Nx_" + str(setup.Nx)
                + "_" + str(setup.T0) + "_" + str(setup.T), setup.t_vec)

        np.save("../../data/aw_hermite/advection/alpha_e_Nv_" + str(setup.Nv) + "_Nx_" + str(setup.Nx)
                + "_" + str(setup.T0) + "_" + str(setup.T), setup.alpha_e)

        np.save("../../data/aw_hermite/advection/u_e_Nv_" + str(setup.Nv) + "_Nx_" + str(setup.Nx)
                + "_" + str(setup.T0) + "_" + str(setup.T), setup.u_e)